...
export const BlogContents = async () => {
  ...
  return (
    <section className="w-full flex items-start mx-auto max-w-screen-xl px-4 mt-5 gap-4">
      <section className="w-full flex-1">
        ...
        {contents.docs.map(({ id, shortDescription, blogTitle, category }, index) => (
          <div className="mb-4" key={index + ''}>
            <SingleBlogPost id={id} shortDescription={shortDescription} blogTitle={blogTitle} />
          </div>
        ))}
...
const SingleBlogPost: React.FC<{
  ...
  blogTitle: string
}> = ({ id, shortDescription, blogTitle }) => {
  return (
    <section className="w-full p-2 rounded-md bg-accent/40 border border-foreground/30">
      <h2 className="text-lg font-semibold line-clamp-1">{blogTitle}</h2>
      <div className="grid grid-cols-[1fr_auto] gap-4 my-2 items-center">
        <p className="text-sm line-clamp-2 min-h-10">{shortDescription}</p>
